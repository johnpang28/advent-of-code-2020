...
import me.jp.aoc.Day02.Password
import me.jp.aoc.Day02.input

fun main() {

    val rule1: (Password) -> Boolean = { password ->
        val (min, max, letter) = password.policy
        password.value.count { it == letter }.let { it in min..max }
    }

    val answer1 = input.count(rule1)
    println(answer1) // 393

    val rule2: (Password) -> Boolean = { password ->
        val (index1, index2, letter) = password.policy
        listOf(index1, index2).map { if (password.value[it - 1] == letter) 1 else 0 }.sum() == 1
    }

    val answer2 = input.count(rule2)
    println(answer2) // 690
}

object Day02 {

    data class Policy(val n1: Int, val n2: Int, val letter: Char)
    data class Password(val policy: Policy, val value: String)

    private fun String.toPassword(): Password =
        """([0-9]+)-([0-9]+)\s([a-z]):\s([a-z]+)""".toRegex().matchEntire(this)?.let {
            val (n1, n2, s, pw) = it.destructured
            return Password(Policy(n1.toInt(), n2.toInt(), s[0]), pw)
        } ?: throw RuntimeException("Unexpected policy format: $this")

    val input: List<Password> = """
        ...
        6-7 s: xsvmsds
        6-7 n: jbncncnn
    """.trimIndent().lines().map { it.toPassword() }
}