...
import me.jp.aoc.Day04.input
import me.jp.aoc.Day04.toPassports
import me.jp.aoc.Day04.validations

fun main() {
    val passports: List<Passport> = input.toPassports()

    val answer1 = passports.count { it.keys.containsAll(validations.keys) }
    println(answer1) // 239

    val answer2 = passports.count { passport ->
        passport.entries.map { (k, v) -> validations[k]?.matches(v) ?: false }.count { it } == validations.size
    }
    println(answer2) // 188
}
