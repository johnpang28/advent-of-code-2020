...
import me.jp.aoc.Day07.bagCount
import me.jp.aoc.Day07.canCarry
import me.jp.aoc.Day07.input
import me.jp.aoc.Day07.parse

fun main() {
    val allBags: Map<String, Map<String, Int>> = input.lines().map { parse(it) }.toMap()

    val answer1 = canCarry(allBags, "shiny gold").size
    println(answer1) // 224

    val answer2 = bagCount(allBags, "shiny gold")
    println(answer2) // 1488
}
...
object Day07 {
    ...
    fun parse(s: String): Pair<String, Map<String, Int>> {
        ...
        val contains = matchResults.drop(1).fold(emptyList<Pair<String, Int>>()) { acc, n ->
            val (count, inColour) = n.destructured
            if (count.isBlank()) acc else acc + (inColour to count.toInt())
        }.toMap()

        return outColour to contains
    }
