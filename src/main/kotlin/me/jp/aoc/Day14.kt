...
import me.jp.aoc.Day14.input
import me.jp.aoc.Day14.parseInput
import me.jp.aoc.Day14.run
import me.jp.aoc.Day14.run2

fun main() {
    val program = input.parseInput()

    val answer1 = program.run().values.sum()
    println(answer1) // 16003257187056

    val answer2 = program.run2().values.sum()
    println(answer2) // 3219837697833
}

typealias Program = List<Pair<String, List<Pair<Long, Long>>>>

object Day14 {

    fun Program.run(): Map<Long, Long> =
        flatMap { (mask, values) -> values.map { (address, value) -> address to value.mask(mask) } }.toMap()

    private fun Long.mask(mask: String): Long =
        toBinary().zip(mask).map { (b, m) -> if (m == '0' || m == '1') m else b }.joinToString("").toLong(2)

    fun Program.run2(): Map<String, Long> =
        flatMap { (mask, values) -> values.flatMap { (address, value) -> address.mask2(mask).map { it to value } } }.toMap()

    private fun Long.mask2(mask: String): List<String> {
        val masked = toBinary().zip(mask).map { (b, m) -> if (m == '0') b else if (m == '1') '1' else 'X' }
        val initial = if (masked.first() == 'X') listOf(listOf('0'), listOf('1')) else listOf(listOf(masked.first()))
        return masked.drop(1).fold(initial) { acc, n ->
            if (n == 'X') acc.flatMap { listOf(it + '0', it + '1') }
            else acc.map { it + n }
        }.map { it.joinToString("") }
    }

    private fun Long.toBinary(): String = java.lang.Long.toBinaryString(this).padStart(36, '0')

    fun String.parseInput(): Program = this.lines().fold(emptyList()) { acc, n ->
        "^mask = ([X10]{36})$".toRegex().matchEntire(n)?.let {
            val mask = it.groupValues[1]
            acc + (mask to emptyList())
        } ?: """^mem\[([0-9]+)] = ([0-9]+)$""".toRegex().matchEntire(n)?.let {
            val (address, value) = it.destructured
            val (mask, memoryUpdates) = acc.last()
            acc.dropLast(1) + (mask to memoryUpdates + (address.toLong() to value.toLong()))
        } ?: throw RuntimeException("Bad input $n")
    }
